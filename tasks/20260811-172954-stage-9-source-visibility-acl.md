...
> Generated from Claude Code plan: `/Users/hari/proj/gdgjp/docs/plans/09-source-visibility-acl.md`
...
- **移行バックフィルの等価性** — 移行前の `canAccessSource(chapterIds)` と
  移行後の `canAccessSource(chapters)` が、`{admin, owner, 所属あり, 所属なし} ×
  {chapter_id NULL, 非 NULL}` の全組み合わせで一致する。
