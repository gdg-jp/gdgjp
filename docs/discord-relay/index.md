...
> **ただし事実 A は 1 点に従属する。** UC-108 は「READY / `GUILD_CREATE` / `CHANNEL_*` から
> ギルドとチャンネルの一覧を保持し、**ルール編集のセレクタに供給する**」と書いている。
> これを字義どおりに採ると、`03` のチャンネルフィルタ UI が `06` に従属し、
> **`03`〜`05` が Gateway 待ちになって事実 A が崩れる。**
>
> 回避策は既にリポジトリにある。`wiki/app/routes/api/discord/guild-channels.ts` は
> Discord **HTTP API**（ACTOR-006）からチャンネル一覧を引いており、RDRA も
> [この実装パターンを BIZ-002 の下敷きにする](../rdra/discord-relay/overview.md)と書いた。
> **セレクタの供給元を ACTOR-006 に置き、UC-108 を「Gateway イベントによる更新」に格下げするかを
> `02` で決める。** 決めるまで `03` の着手見積もりは確定しない。
...
- `00` の SETUP-2（特権 Intent）は Discord の審査が挟まりうる。**着手を後ろに倒さない**
- `04 → 06` は上記のとおり契約先行
...
| 論点 | 決める場所 | 決め方 |
|---|---|---|
| 初期に有効化する特権 Intent（SETUP-2） | **00** | 審査が挟まりうるので最初に確定させる |
...
| `169.254.169.254` が VCN DNS を兼ねるか | **07** | 実機確認。`IPAddressDeny=link-local` の可否がこれに従属する |
...
**`00` の Discord Application は取り消しにくい。** 特権 Intent の申請、100 サーバー到達前の
...
`00` で**予算アラートを設定する**こと。A1 は合計 4 OCPU / 24 GB、ブロックストレージ 200 GB を
超えた分が課金される。
