...
| API response model | Synchronous JSON everywhere, **except** tinyurl's Vercel domain provisioning and sns's "publish now to X" — both call slow external APIs and use the async Job+Queue pattern connpass already established. |
...
| 4 | Scheduled publish logic could no-op, swallow failure, or race cron | Stage 13 adds an atomic `publishNow` transition with explicit outcomes, active-job uniqueness, and shared X-posting internals. |
...
| 6 | Async jobs lacked ownership and recovery rules | Job reads derive authorization from their resource; send failure, redelivery, terminal no-op, and stale-running lease recovery are specified. |
| 7 | sns post/media mutations were not one aggregate | Stage 10 centralizes draft/media transitions and defines R2/D1 compensation and ordering semantics. |
| 8 | Campaign analytics could leak participant data and had no bounds | Only bounded aggregate analytics are exposed; participant ids, snapshots, and raw events are excluded. |
| 9 | img chapter selection and `updatedAt` were ambiguous | Multi-chapter callers must select a chapter; update responses use the persisted row returned by D1. |
| 10 | CLI helper cleanup risked changing existing behavior | Existing Accounts/agent compact output and error behavior are frozen byte-for-byte; only new commands and already-indented connpass output use `PrintJSON`. |
| 11 | Campaign channel/source management was incomplete | Channel and source update/archive/restore routes and CLI commands are part of Stage 08/09. |
| 12 | SNS CLI-created posts could lose dashboard link previews | Stage 10 keeps preview derivation from final post text inside the aggregate service; Stage 12 does not accept client-authored preview metadata. |

## Three-layer architecture

1. **Shared groundwork** (Stages 01–02 and 07A) — shared bearer identity plus a
   CLI-scope-validating Accounts identity endpoint/consumer, a Go `cliutil`
   package consolidating logic duplicated across the CLI's existing per-app command packages.
   Stage 07A contains only app-neutral async-job types/state transitions/serialization.
...
| 07A | `07a-shared-job-primitives.md` | — | ✅ |
| 07 | `07-tinyurl-domains-bearer-api.md` | 01, 05, 07A | |
...
| 13 | `13-sns-x-publish-async.md` | 12, 11, 07A | |
| 14 | `14-sns-cli-client.md` | 12, 13, 02 | |

**Stages 01, 02, 05, 07A, 10, 11 have no dependencies on each other or on anything else** and can all
begin immediately, in parallel, across separate sessions if wall-clock time
matters. Everything else branches off those six.

Recommended sequencing if running mostly serially: 01 → 02 → (img: 03 → 04) → (tinyurl: 05 → 06 →
07A → 07 → 08 → 09) → (sns: 10 & 11 & 07A → 12 → 13 → 14). img goes first because it's the cleanest of the
...
- **Async Job envelope** (tinyurl domain provisioning, sns X-publish only): modeled on connpass's
  `Job` shape (`app/lib/jobs.server.ts`'s `jobToJson`) — `{ id, type, status, request, result,
  error, createdBy, createdAt, updatedAt, startedAt, finishedAt }`, returned as `202` from the
  triggering endpoint and re-fetchable from `GET /api/cli/v1/jobs/:jobId`. Job reads require
  creator or owning-resource/chapter authorization; ids are not treated as authorization.
  Stage 07A defines only the app-neutral lifecycle vocabulary and serialization contract. It does
  not migrate connpass and does not make SNS depend on TinyURL provisioning.
...
- **tinyurl**: list active domains → create/list/get/update/delete a link; register a domain →
  wait/inspect → list/get/sync/delete it; create/list a campaign → discover channels and sources →
...
- **sns**: list usable X accounts → create/list/get/update a draft → add/remove media → publish now
  and wait → inspect terminal post/job state; organizers can list/add/remove contributors.
