...
function withPrefix(prefix: string): string {
  return `${prefix}${encodeBase32(uuidv7Bytes())}`;
}

export function newEventId(): string {
  return withPrefix("evt_");
}

export function newClaimId(): string {
  return withPrefix("clm_");
}

export function newItemId(): string {
  return withPrefix("itm_");
}

export function isEventId(value: string): boolean {
  return /^evt_[0-7][0-9A-HJKMNP-TV-Z]{25}$/.test(value);
}

export function isClaimId(value: string): boolean {
  return /^clm_[0-7][0-9A-HJKMNP-TV-Z]{25}$/.test(value);
}
