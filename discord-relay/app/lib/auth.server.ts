...
/**
 * The GDG Accounts relying-party auth instance for Discord Relay.
 *
 * Cookie prefix `gdgjp-discord-relay` isolates Discord Relay's session/tx cookies from
 * the other apps on `*.gdgs.jp`. OIDC discovery/token/UserInfo calls go through the
...
export function getAuth(env: Env): RpAuthInstance {
  ...
  const instance = initializeRpAuth({
    db: env.DB,
    appUrl: env.APP_URL,
    cookiePrefix: "gdgjp-discord-relay",
