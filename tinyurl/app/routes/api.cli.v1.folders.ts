...
import { createFolder, listAccessibleFoldersPage } from "~/features/folders";
...
function cursor(raw: string | null) {
  try {
    const offset = Number(raw ? atob(raw) : 0);
...
export async function loader(args: Route.LoaderArgs) {
  ...
  const parent = id(url.searchParams.get("parentFolderId"));
  const rawLimit = url.searchParams.get("limit");
  const limit = rawLimit === null ? 50 : Number(rawLimit);
  const offset = cursor(url.searchParams.get("cursor"));
  if (parent === null || offset === null || !Number.isInteger(limit) || limit < 1 || limit > 100)
    return cliError("invalid_request", 400);
  return cliJson(
    await listAccessibleFoldersPage(env.DB, viewer(auth), {
...
export async function action(args: Route.ActionArgs) {
  ...
  const { name, parentFolderId } = parsed.value;
  if (
    typeof name !== "string" ||
    !name.trim() ||
    name.trim().length > 48 ||
    (parentFolderId !== undefined &&
      (!Number.isInteger(parentFolderId) || (parentFolderId as number) <= 0))
  )
    return cliError("invalid_request", 400);
  const result = await createFolder(env.DB, {
    name: name.trim(),
    actor: viewer(auth),
    ...(typeof parentFolderId === "number" ? { parentFolderId } : {}),
  });
  if (!result.ok)
    return cliError(
  ...
  return cliJson({ folder: result.folder }, { status: 201 });
}
