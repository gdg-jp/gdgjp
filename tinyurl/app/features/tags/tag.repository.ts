...
export async function createTag(db: D1Database, input: CreateTagInput): Promise<TagMutationResult> {
  try {
    const row = await db
      .prepare(
        `INSERT INTO tags (name, color, owner_user_id, owner_chapter_id) VALUES (?, ?, ?, ?) RETURNING ${TAG_COLS}`,
      )
      .bind(
        input.name,
        input.color ?? null,
...
export async function updateTag(
  db: D1Database,
  input: { id: number; name: string; color?: string | null },
): Promise<TagMutationResult> {
  try {
    const row = await db
      .prepare(`UPDATE tags SET name = ?, color = ? WHERE id = ? RETURNING ${TAG_COLS}`)
  ...
  const { results } = await db
    .prepare(
      `SELECT ${TAG_COLS.split(", ")
        ...
        .join(
          ", ",
        )}, (SELECT COUNT(*) FROM link_tags lt JOIN links l ON l.id = lt.link_id WHERE lt.tag_id = t.id AND l.archived_at IS NULL AND l.deleted_at IS NULL) AS link_count FROM tags t WHERE ${clause} ORDER BY t.name`,
    )
    .bind(value)
    .all<TagRow & { link_count: number }>();
  return results.map((row) => ({ ...toTag(row), linkCount: row.link_count }));
}
export function listTagsForUserWithCounts(db: D1Database, userId: string) {
  return listWithCounts(db, "t.owner_user_id = ?", userId);
}
export function listTagsForChapterWithCounts(db: D1Database, chapterId: number) {
  return listWithCounts(db, "t.owner_chapter_id = ?", chapterId);
}
