-- See accounts/migrations/0012_simplify_user.sql for full notes on why this
-- uses in-place ALTER TABLE rather than create-new/drop-old (the latter
-- cascade-deletes child rows that reference user.id ON DELETE CASCADE under
-- D1's transactional migration wrapper).
