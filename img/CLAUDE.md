...
- Public GET (`routes/$id.tsx`): no auth. Plain URLs automatically negotiate AVIF/WebP and optionally scale down to `IMG_AUTO_MAX_WIDTH`; `?f=original` returns the uploaded bytes. `?radius=<px>` makes raster-image corners transparent (and forces PNG when combined with `f=jpeg`). Canonical normalized renditions persist in `DERIVED`, with a synthetic Cache API key in front. ETags include variant, source version, normalized params, and format. Images failures fall back to originals rather than returning 500.
