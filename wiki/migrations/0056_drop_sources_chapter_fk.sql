-- /sources picks chapter scope from the Accounts directory (and IdP memberships),
-- not wiki's local `chapters` table, which is not kept in sync. The FK made every
-- chapter-member / chapter-organizer insert fail with a D1 foreign-key error whenever
-- the chosen Accounts id was missing locally (common in local/dev and empty caches).
--
-- Chapter ids remain opaque Accounts identifiers; visibility CHECK still requires
-- chapter_id exactly when visibility is chapter-scoped.
